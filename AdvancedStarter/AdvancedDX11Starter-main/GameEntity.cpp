...
std::shared_ptr<Transform> GameEntity::GetTransform() { return std::make_shared<Transform>(&transform); }
...
void GameEntity::Draw(Microsoft::WRL::ComPtr<ID3D11DeviceContext> context, std::shared_ptr<Camera> camera)
{
	...
	material->PrepareMaterial(std::make_shared<Transform>(&transform), camera);
